...
function initMinigames() {
    ...
    cultist_minigame.createHTML = function () {
        ...
        ritual_soot.attr("onmouseover", "tooltip('#'+this.id, 5, 1, 'Ritual of Soot', 'Increases production by 15% + 3% for each previous ritual of soot activation <span style=\"color:#00db0a;\">('+Math.round((0.25 + minigames[0].vars.soot_counters * 0.03) * 100)+'%)</span>. This effect lasts 2 minutes. Each activation also permanently decreases production 1% <span style=\"color:#ff1e2d;\">(-'+Math.round(minigames[0].vars.soot_counters)+'%)</span>.<br><span>Cost: 95 Blood</span><br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_4.toUpperCase() + ' </span>')");
    ...
    cultist_minigame.update = function (dt) {
        ...
        vars = this.vars;
        vars.blood += dt/2 * vars.max_blood/200;
        vars.max_blood = Math.floor(100 + buildings[0].count/4) + minigames[5].vars.research_tree[12].bought * 6;
		if (Math.abs(vars.blood - vars.max_blood) < 0.5 || (dt >= 10 && vars.blood > vars.max_blood)) {vars.blood = vars.max_blood}
		if (vars.blood > vars.max_blood) {vars.blood -= dt * 2 * vars.max_blood/200;}
    };
	...
	mine_minigame.upgrades = [
		...
		[183, function () {return true;}, "Unlocks once you spend more than three days playing."],
	...
	gambler_minigame.createDetails = function () {
        ...
        cards.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
			...
			if (upgrades[38].bought) {
				cards_string += "J&spades;: <span style='color:#ff8300;'>Instantly grants 20 seconds worth of production.</span><br>";
				cards_string += "J&clubs;: <span style='color:#ff8300;'>Instantly clicks 20 times.</span><br>";
			}
	...
	gambler_minigame.update = function (dt) {
		dt *= 1 + 0.01 * minigames[5].vars.research_tree[20].bought;
		
		this.vars.draw_charges_max = (5 + minigames[5].vars.research_tree[14].bought) * (upgrades[39].bought + 1);
	...
	power_minigame.update = function (dt) {
		...
		this.vars.power_rate = (0.5 + (buildings[3].count / 300)) + (upgrades[49].bought * 0.25) + (upgrades[195].bought * 0.25);
	...
	bank_minigame.createHTML = function () {
		...
		investment_container.attr("style", "position:relative");
			...
			investment_container.attr("onmouseover", "investTooltip(this)");
	...
	research_minigame.vars = {
		...
		research_tree: [
			...
			new Research("Epiphanic Stockpile", "Will grant 10 Research points upon resetting.", function () {return ""}, 6, 2, 78, 374, 5, 5, function () {}, function () {return true}, 17), //24
	...
	research_minigame.update = function (dt) {
		...
		if (this.vars.research_time <= 0) {
			this.vars.research_time_max = 60 - minigames[5].vars.research_tree[5].bought;
	...
	bonus_minigame.update = function (dt) {
		...
		if (this.vars.bonus_time <= 0) {
			this.vars.bonus_time = this.vars.bonus_max_time - upgrades[113].bought * 15;
	...
	click_minigame.update = function (dt) {
		...
		this.vars.max_clicks = Math.floor(20 + buildings[8].count * 0.5 + upgrades[123].bought * 15);
	...
	cryogenic_minigame.upgrades = [
		...
		//[113, function () {return true;}, "Unlocks once you warp forward 10 times."],
		//[114, function () {return true;}, "Unlocks once you warp forward 25 times."],
		//[115, function () {return true;}, "Unlocks once you warp forward 50 times."],
		//[116, function () {return true;}, "Unlocks once you warp forward 100 times."],
	];
	...
	alien_minigame.update = function (dt) {
		this.vars.max_power = 60 + Math.floor(buildings[10].count * 0.5) + upgrades[143].bought * 30;
	...
	computer_minigame.createHTML = function () {	
		...
		var looper = $(document.createElement("img"));
		looper.attr("class", "loop").attr("id", "computer_loop").css("cursor", "pointer");
		if (!minigames[11].vars.looping) {
			looper.attr("src", "images/misc_loop_inactive.png");
		} else {
			looper.attr("src", "images/misc_loop.png");
		}
		looper.attr("height", "48").attr("width", "48");
		looper.attr("onclick", "toggleComputerLoop()");
		looper.attr("onmouseover","tooltip(this, 7, 16, 'Program Looping', 'Toggle to cause programs to restart after finishing.', function () {}, true)");
		looper.attr("onmouseout", "hideTooltip();");
		
		$("#building_tab_sidebar").append(looper);
	...
	computer_minigame.update = function (dt) {
		...
		if (this.vars.program_1 != 0) {
			...
			if (this.vars.program_1_time <= 0) {
				...
				else if (this.vars.program_1 == 4 && upgrades[154].bought && this.vars.looping) {this.vars.program_1 = 0;looped = true;program1();}
				else if (this.vars.program_1 == 4 && upgrades[154].bought) {this.vars.program_1 = -1;}
		...
		if (this.vars.program_2 != 0) {
			...
			if (this.vars.program_2_time <= 0) {
				...
				else if (this.vars.program_2 == 4 && upgrades[154].bought && this.vars.looping) {this.vars.program_2 = 0;looped = true;program2();} 
				else if (this.vars.program_2 == 4 && upgrades[154].bought) {this.vars.program_2 = -1;}
		...
		if (this.vars.program_3 != 0) {
			...
			if (this.vars.program_3_time <= 0) {
				...
				else if (this.vars.program_3 == 4 && upgrades[154].bought && this.vars.looping) {this.vars.program_3 = 0;looped = true;program3();} 
				else if (this.vars.program_3 == 4 && upgrades[154].bought) {this.vars.program_3 = -1;}
	...
	epiphany_minigame.updateHTML = function () {
		...
		$("#epiphany_display").html("Next Epiphany: " + Math.round(this.vars.epiphany_time) + "s" + target_text);
	...
	epiphany_minigame.updateDetails = function () {
		...
		stats_string += "Base Production: <span style='color:#00db0a;'>" + fancyNumber(buildings[15].production * buildings[15].count) + "/s</span><br>&nbsp;&nbsp; <span style='color:#00db0a;'>("+ fancyNumber(buildings[15].production) +" per cloning lab)</span><br>";
	...
	merchant_minigame.createDetails = function () {
        ...
        description.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
            ...
            description.append(description_header);
            var description_content = $(document.createElement("div"));
            description_content.html("&nbsp;&nbsp;&nbsp;Once every 5 minutes grants currency of a random building."); 
            description_content.css("text-align", "left");
            description_content.toggle();
            description.append(description_content);
	...
	warp_minigame.upgrades = [
		[107, function () {return true;}, "Unlocks once you own 10 warp facilities."],
		[108, function () {return buildings[7].count >= 10;}, "Unlocks once you own 25 warp facilities."],
		[109, function () {return buildings[7].count >= 25;}, "Unlocks once you own 50 warp facilities."],
		[110, function () {return buildings[7].count >= 50;}, "Unlocks once you own 75 warp facilities."],
		[111, function () {return buildings[7].count >= 75;}, "Unlocks once you own 100 warp facilities."],
		[112, function () {return buildings[7].count >= 100;}, "Unlocks once you own 125 warp facilities."],
		[113, function () {return true;}, "Unlocks once you warp forward 10 times."],
		[114, function () {return true;}, "Unlocks once you warp forward 25 times."],
		[115, function () {return true;}, "Unlocks once you warp forward 50 times."],
		[116, function () {return true;}, "Unlocks once you warp forward 100 times."],
	...
	if (upgrades[113].bought) {minigames[17].vars.warp_max_charges = 15}
	if (upgrades[115].bought) {minigames[17].vars.warp_max_time = 165}
	...
	stellar_minigame.createDetails = function () {
		...
		upgrades_tab.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
			...
			for (var i = 0; i < minigames[18].upgrades.length; i++) {
				upgrades_string += stringUpgrade(minigames[18].upgrades[i], 8, 10);
			}
	...
	temporal_minigame.createHTML = function () {
		...
		var temporal_bottle = $(document.createElement("img"));
			...
			temporal_bottle.addClass("temporal_active");
		...
		var temporal_experience = $(document.createElement("img"));
			...
			temporal_experience.attr("onmouseover", "tooltip('#'+this.id, 0, 29, 'Quick Experience', 'Toggle to double the rate that assistants gain progress towards the next level.<br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_5.toUpperCase() + ' </span>')");
	...
	temporal_minigame.createDetails = function () {
		...
		upgrades_tab.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
			...
			for (var i = 0; i < minigames[19].upgrades.length; i++) {
				upgrades_string += stringUpgrade(minigames[19].upgrades[i], 8, 10);
			}
	...
	temporal_minigame.updateDetails = function () {
		...
		stats_string += "Base Production: <span style='color:#00db0a;'>" + fancyNumber(buildings[19].production * buildings[19].count) + "/s</span><br>&nbsp;&nbsp; <span style='color:#00db0a;'>("+ fancyNumber(buildings[19].production) +" per stellar factory)</span><br>";
		stats_string += "Stellar Factories Owned: <span style='color:#ff8300;'>" + buildings[19].count + "</span><br>";
