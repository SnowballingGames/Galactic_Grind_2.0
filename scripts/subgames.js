...
function initSubgames() {	
	...
	corruption_subgame.calculateProduction = function () {
		...
		for (var i = 0; i < this.buildings.length; i++) {
			...
			this.vars.seed_production += this.buildings[i].seed_production * this.buildings[i].count;
		}
...
function updateSubgameButtons() {
	...
	if (subgames[0].unlocked || assistants[0].unlocked || assistants[1].unlocked || assistants[2].unlocked) {
		$("#subgame_title").show();
