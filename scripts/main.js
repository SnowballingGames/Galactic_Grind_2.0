...
function fastTick() {
    ...
    //Slow Tick?
    updateTooltip();
    $("#popup_text").css("opacity", $("#popup_text").css("opacity")-dt/3);
    $("#popup_achievement").css("opacity", $("#popup_achievement").css("opacity")-dt/3);
...
function increaseSpeed() {
    ...
    if (GAME_SPEED == 1) {updateGameSpeed(2)}
		...
		if (minigames[5].vars.research_tree[3].researched) {
			updateGameSpeed(5 + minigames[5].vars.research_tree[3].researched);
		} else {
			updateGameSpeed(1);
		}
	}
...
function updateGlobalDisplay() {
	if ($("#global_container").is(":visible")) {
		...
		var settings_string = "";
		var auto_10 = "", auto_30 = "", auto_endless = "", skip_tutorial = "", effects = "", menu = ""; 
